...
void main() {
  group('FaceEmbedder Tests', () {
    ...
    test('maintains high similarity on smudged/low-contrast image of same subject', () {
      const size = 112;
      ...
      for (var y = 0; y < size; y++) {
        for (var x = 0; x < size; x++) {
          final idx = y * size + x;
          bytesA1[idx] = 120;
          if (y >= 20 && y <= 40 && x >= 25 && x <= 87) bytesA1[idx] = 50; // Eyes region
          if (y >= 45 && y <= 70 && x >= 45 && x <= 67) bytesA1[idx] = 200; // Nose region
          if (y >= 75 && y <= 90 && x >= 35 && x <= 77) bytesA1[idx] = 70; // Mouth region
        }
      }
    ...
    test('produces low similarity between distinct subjects (Person A vs Person B)', () {
      const size = 112;
