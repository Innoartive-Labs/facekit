...
void main() {
  ...
  for (var y = 0; y < size; y++) {
    for (var x = 0; x < size; x++) {
      final idx = y * size + x;
      bytesA1[idx] = 130;
      if (y >= 25 && y <= 40 && x >= 25 && x <= 87) bytesA1[idx] = 45; // Eyes region
      if (y >= 45 && y <= 70 && x >= 45 && x <= 67) bytesA1[idx] = 210; // Nose region
      if (y >= 78 && y <= 92 && x >= 35 && x <= 77) bytesA1[idx] = 65; // Mouth region
    }
  }
  ...
  for (var y = 0; y < size; y++) {
    for (var x = 0; x < size; x++) {
      final idx = y * size + x;
      bytesB[idx] = 75;
      if (y >= 15 && y <= 30 && x >= 15 && x <= 45) bytesB[idx] = 220; // Left feature
      if (y >= 55 && y <= 85 && x >= 55 && x <= 95) bytesB[idx] = 35; // Right feature
    }
  }
  ...
  print('Intra-class similarity (Same Subject A clear vs smudged): ${(intraSim * 100).toStringAsFixed(2)}%');
  print('Inter-class similarity (Subject A vs Subject B): ${(interSim * 100).toStringAsFixed(2)}%');
  print('Discriminative Margin: ${((intraSim - interSim) * 100).toStringAsFixed(2)}%');
